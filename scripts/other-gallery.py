...
# or apt install python3-imagesize
...
# configuration
output_file = "other.yml"
input_file = output_file
image_path = "other"
extensions= ['jpg', 'png']

# set correct path
path = join("../../assets/astrophotography", image_path)
...
if isfile(input_file):
    with open(input_file, 'r') as f:
        input_gallery = yaml.load(f, Loader=yaml.SafeLoader)
else:
...
for pic in new_gallery:
    ...
    if not found:
        # create new entry
        object_name = pic.split(sep="-", maxsplit=1)[0].upper()
        title = object_name.replace('-', ' ').replace('_', ' ').title()
        old_gallery.append({"filename": pic, "sizes": new_gallery[pic], "thumbnail": thumbs[pic], "original": originals[pic], "title": title, "sort_id": sort_id})
        sort_id += 1
        
input_gallery['pictures'] = sorted(old_gallery, key=itemgetter('sort_id'))

for i, pic in enumerate(input_gallery['pictures'], start=1001):
    pic['sort_id'] = i
